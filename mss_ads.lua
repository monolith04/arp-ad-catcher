 ...
 --debug(t_type, 4)
  if t_type == "my_change" or t_type == "ur_change" or t_type == "change" then
    ...
    elseif str:find(" на [^(вв)]") then
      ...
      print("ОБМЕН (секция 3): ",action[t_type], h_type, h_class, h_repair, h_number, h_location, get_price(str, t_type))
...
function home_changer(str)
  ...
  elseif str:find("дом на [вВB][вВB]") then
    return " на дом на г. VineWood"
  elseif str:find("дом у вмф") then
    return " на дом в д. Bayside"
  elseif str:find("в гетто") or str:find("[Оо]пасном") then
  ...
  elseif str:find("[ПпPp][KkКк]") or str:find("[Пп][ао]л[д]?[оа]м[ио][нк][оа]") or str:find("(.+)[ao]lo[nm][n]?ino%p?%s?(.+)re[e]?[ea]k") then
...
function get_house_type(str)
  ...
  elseif str:find("[тТ]ру[щш][ео]б[уыа]?") then
    home_type = "трущебу"
  elseif str:find("[Аа][пп]артаменты") or str:find("[Аа]парты") then
    home_type = "апартаменты"
  elseif (str:find("[Ээ]лит") and str:find("[Аа]п[п]?арт")) then
    home_type = "Элитные Аппартаменты"
...
function get_house_class(str)
  ...
  elseif (str:find("[Ээ]лит") and str:find("[Аа]п[п]?арт")) then
    house_class = ""
  elseif str:find("[ЭэЕе]к[оа]") or str:find("низкого") or str:find("[Ee]conom") then
...
function get_house_repair(str)
  ...
  elseif str:find("[рР]ем[но][он]т") or str:find("интой") or str:find("инта") or str:find("ИНТОЙ") or str:find("интерьером") then
...
function get_location(str)
  if str:find("па[см][Сс]?[Тт]?но[гмй]") or str:find("[Гг][еэ][т]?т[о]?[т]?[оа]") or str:find("ГЕТТО") or str:find("[Гг]етт") or str:find("[Gg][h]?et[t]?o") or str:find("%sг[еэ]тт%s") or str:find("пасны[мй]") or str:find("оп р") or str:find("опастом") then
    ...
    elseif str:find("на горе вагос") or str:find("[Гг]ор[еа] [Вв]агос") or str:find("в районе [Vv]agos") or str:find("%p[Vv]agos%p") or str:find("[Тт]ер[ие]тории [Вв]агос") then
  ...
  elseif str:find("[лЛ]с") or str:find("ЛС") or str:find("лос") or str:find("Лос") or str:find("ш.LS") or str:find("%s[Ll][Ss]") or str:find("LS%s?^[|]") or str:find("Los") or str:find("los") then
    ...
    elseif str:find("[Аа]эропорт") then
  ...
  elseif not str:find("куебра") and str:find("[Ллl][BВвv]") or str:find("%sлас") or str:find("Лас") or str:find("ш.L%p?V") or str:find("%sL.V.") or str:find("%sLV") or str:find("[Bb][Ll][Vv]") or str:find("LV[($)|(.)]") or str:find("LVPD") or str:find("enturas") or str:find("ентура") then
    if str:find("[Бб]ол[ь]?[нк]") or str:find("[БбBb][ЛлLl][ВвVv]") then
  ...
  elseif str:find("[Тт][иь]?[Рр]?[Ееа][рг][р]?[ао][^с]") or str:find("[Tt]ier[r]?[oa]") or str:find("[Tt]iro [Rr]obada") or str:find("%s[Тт][Рр]%s") or str:find("[TТ]%s[(Робада)|Robada]") then
    obj_location = " в о. Tierra Robada"
  elseif str:find("[ВвB][вВB][^сС]") or str:find("ВВ[^С]") or str:find("на горе вв") or str:find("на [г]?%p?[ВвB][BВв]$") or str:find("[wW][Ww]") or str:find("(.+)ine(.+)ood") or str:find("(.+)а[ий][н]?(%s?)(.+)у[н]?д") or str:find("[Вв]ин [Вв]уд") then
    obj_location = " на г. VineWood"
  elseif str:find("[Дд]жунипер [Хх]ол[л]?оу") or str:find("Juniper Hollow") then
    obj_location = " в р. Juniper Hollow, ш. SF"
  elseif str:find("[Ee]ast [Bb]each") then
    obj_location = " в р. East Beach, ш. LS"
  elseif str:find("т[г]?омери") or str:find("[Mm]on[tgG][h]?[tgG][oe]m[eo]r[ey]") or str:find("[мМ]он[о]?[г]?омери") or str:find("M[ao]ntogomery") or str:find("фарм.+%sфаб") or str:find("[Мм]о[н]?[т?][о]?г[еуо]м[ое]р[р]?[иы]") or str:find("мандгомери") or str:find("mogomtery") or str:find("[Мм]отенегри") or str:find("монтенегро") or str:find("[Mm]on[t]?go") or str:find("МОНТОМЕРИ") then
    obj_location = " в д. Montgomery"
  elseif str:find("[Rr]ed") and str:find("[Cc]o[u]?nt[r]?y") or str:find("RedCount[r]?y") or str:find("[Рр]ед%s?[Кк][ао][у]?нтри") or str:find("о.Red") or str:find("[Оо]круг[е]? [Рр]ед") then
  ...
  elseif str:find("[IiLl]dl[e]?wood") then
    obj_location = " в опасном р. Idlewood"
  elseif str:find("[Pp]ri[n]?[c]?[kl][lk][e]?") or str:find("[Пп]ри[кн]?[нк]?л") or str:find("[Пп]ринцил") or str:find("%s[Пп]райкс%s[Пп]райн") or str:find("%sприк%s") or str:find("[Pp][r]?i[n]?[c]?[k]?le") or str:find("[Лл][Кк][Нн]") or str:find("[Ll][CcKk][Nn]") or str:find("[Ии]тал") and ("[Пп]осольства") then
    obj_location = " в р. Prickle Pine"  
  elseif str:find("[Ee]l [Cc]orona") then
    obj_location = " в LS, р. El Corona"
  ...
  elseif str:find("[Qq][Uu][ea][a]?[br]") or str:find("[Кк][аув]?[еа]?[бпр]?[пб]рад[оа]с") or str:find("[Кк][аув]?[еа]?бардос") or str:find("[Кк]арбенос") or str:find("[Кк]вадрос")then
    obj_location = " в д. El Quebrados"  
  elseif str:find("[Tt][ea]mple") or str:find("[Тт][еи]м[лп][лп][е]?") or str:find("ТЕМПЛ") then
    obj_location = " в р. Temple" 
  elseif str:find("(.-)н[д]жел") or str:find("[Аа]нг[е]?[й]?л [Пп][а]?й") or str:find("[Ээ]нджел") or str:find("[Аа]нгел [Пп][еаэи][йн][не]") or str:find("(.+)[Nnh]gel%p?%s?(.+)[ie][ni][en]") or str:find("(.+)ngel%p?(.+)ain")or str:find("[Дд]еревн[яе] за [Шш]ахтой") then
    obj_location = " в д. Angel Pine"
  elseif str:find("[ПпPp][KkКк]") or str:find("[Пп][ао]л[д]?[оа]м[ио][нк][оа]") or str:find("(.+)[ao]lo[nm][n]?ino%p?%s?(.+)r[ei][e]?[eac][kc]") then
    obj_location = " в д. Palomino Creek"
  ...
  elseif str:find("на горе вагос") or str:find("Горе Вагос") or str:find("[Тт]ер[ие]тории [Вв]агос") or str:find("%p[Vv]agos%p") or str:find("[Рр]еспа [Вв]агос")  then
    obj_location = " в опасном р. Los Flores"
...
function house_number(str)
  ...
  elseif str:find("дом №%d+%s") then
    h_number = " №"..str:match("дом №(%d+)%s")
...
function ad_handler(str)
 --debug(str, 2)
  if str:find("[СсCc][Ее]?ред[ня][ян]%s?к") or str:find("[Сс]реднего") or str:find("[Сс]ердный") or str:find("[^(Работает)]%s[Ээ]лит[кн][оауиы]") or str:find("ЭЛИТК") or str:find("[Ээ]к[оа]номк[ау]") or str:find("[Ээ]к[оа]ном[^ь]") or str:find("[Вв]ысокий класс") or str:find("[Сс]редний класс") then
    -- debug("CATCHED", 3)
    if str:find("[Аа]у[кц]") then
       return auction(str)
    end
  ...
  elseif not str:find("[Сс]ет") and not str:find("pacey") and not str:find("[Сс]имку") and not str:find("24%p7") and not str:find("роймат") and not str:find("[Gg][Pp][Ss]") and not str:find("адовых") and not str:find("[Рр]иелт") and not str:find("[Сс]кин") and not str:find("GPS 8%p2") and not str:find("антех") and str:find("%s[Дд][;жл]?[МмОоЛл][МмСсОо]") or str:find("^[Дд]ом%s") or (str:find("[Ээ]лит") and str:find("[Аа]п[п]?арт")) or str:find("^%W- дои$") or str:find("[Пп]родам %pДом") or str:find("%W+%pдом%s") or str:find("плю до в") or str:find("дамдом") or str:find("жильё") or str:find("%shouse") or str:find("картиру") or str:find("[Вв]илла") or str:find("%shome") or str:find("[Вв]иллу") or str:find("dom") or str:find("[сС]арай") or str:find("[Кк]аварт") or str:find("%sкв%s") or str:find("[Ээ]лит(.-) [Рр]езид(.-)") or str:find("[Шш]алаш") or str:find("[Мм]ини%pдом") or str:find("[Вв][ро]ем[я]?нк[уа]") or str:find("врем[е]?няку") or str:find("времяну") or str:find("ВРЕМЕННОЕ") or str:find("[Вв]рем[ея]н") or str:find("[Кк][Вв][АаОо][Тт]?[Рр]") or str:find("%sхат") or str:find("[Вв][ао]гончик") or str:find("%sтрел") or str:find("[Тт]рейл") or str:find("жилье") or str:find("мини%p?%s?дом") or str:find("плю дам") or str:find("дам [Аа]партаменты") or str:find("[аА]парты") or str:find("[Пп]оместье у озера") or str:find("vremenoe jilie") or str:find("[тТ]ру[щш][ео]б[уыа]?") or str:find("[Хх]ижин[ау]") or str:find("[Оо]соб[ня][ян]к") or str:find("избу") or str:find("[Мм]отель") then
    --debug("HATA", 5)
    if str:find("[Аа]у[кц]") or str:find("[Нн]а аук[е]?") or str:find("[Аа]ууционе") then
      house_type = get_house_type(str)
  ...
  elseif str:find("[Лл]юб[оа][йя] [Мм]арк[иа]") or str:find("[Кк]уплю [аА]втомобиль%s?%p?$") or str:find("[Лл]юб[оа][йя] [FfФф][TtТт]") or str:find("[FfФф][TtТт] автомобиль") or str:find("люб[оу][ею] [(авто)|(тачку)]") or str:find("[Кк]уплю авто%p") or str:find("[Кк]уплю [Мм]аш[иы]н[уы]%p?$") or str:find("[Кк]уплю [Мм]аш[иы]ну [Бб]юд") or str:find("вертолёт%.") or str:find("самол[её]т любой") then
  ...
  elseif str:find("[мМ][еоа]?в[ае]р") or str:find("маврик") or str:find("a[vw]er[r]?i[c]?[kc]") or str:find("мавик") or str:find("mavik") or str:find("ав[ео][р]?ик") or str:find("averiс") then 
    return vechicles(str, trade_type(str), "вертолёт", "Maverick")
  elseif str:find("[zZЗз][rRРр]%p?%s?2000") then 
  ...
  elseif str:find("[Rr][ea]mington") or str:find("[Рр]емин[г]?тон") then
    return vechicles(str, trade_type(str), "автомобиль", "Remington")
  elseif str:find("[Ee]speran[td]o") or str:find("[ЕеЭэ]сперанто") then
  ...
  elseif str:find("[Тт][Цц]?[Уу][Рр][Ии][Кк]%s?+") or str:find("[Тт]ури[зс]м[ао]%s?+") or str:find("[Tt][UuYy]ri[sz]mo%s?+") then
  ...
  elseif str:find("[Gg]re[e]?n[wv]") or str:find("[Гг][р]инвуд") then
  ...
  elseif str:find("[Mm]onst[e]?r") or str:find("[Мм]%s?онст[еа]?[р]?") or str:find("MONSTER") or str:find("МОНСТ[Е]?Р") then
  ...
  elseif str:find("[Bb]ob[ckKC][ea]t") or str:find("[Бб]обк[аеэ]т") then
  ...
  elseif str:find("[Ff][BbIi][BbIiRr]%s?%p?[Tt]r[ua]ck") or str:find("[Фф][БбИи][БбИиРр] [Тт]р[ау][к]?") or str:find("F[BI][IBR] TRUCK") then
  ...
  elseif str:find("[Бб]ур[р]?ит[т]?о") or str:find("[Bb]ur[r]?it[t]?o") then
  ...
  elseif str:find("[Aa][n]?drom[ae]da") or str:find("[Аа]ндр[оа]м[аеэ]д[ау]") then
  ...
  elseif not str:find("[GgГгЖж][PpПп][SsСс]") and (str:find("сперроу") or str:find("спар[р]?оу") or str:find("a[r]?row") or str:find("[Ss]appraw") or str:find("сперов") or str:find("спар[р]?ов") or str:find("СПАР[Р]?ОВ")) then
    return vechicles(str, trade_type(str), "вертолёт", "Sparrow")
  elseif str:find("[Rr][ae]in[e]?d[ea]nc[c]?e") or str:find("[Рр]ейнда") then
  ...
  elseif str:find("arq[ui][ie]?s") or str:find("маркиз") or str:find("[мМ]арки[Уу]?с") then
  ...
  elseif str:find("[Rr]e[ae]?fe[e]?r") or str:find("[Рр]ифер") then
  ...
  elseif str:find("[Ss]pe[e]?de[e]?r") or str:find("[Сс]пидер") then
  ...
  elseif str:find("nf[er][er]nus%s?+") or str:find("н[ф]?е[ру][нр][ун]с%s?+") or str:find("инф[ау]") or str:find("[Ii]nf[eu](.-)s%s?+") or str:find("[иИ][Нн][Фф][Ее][Рр]%s?+") or str:find("инф[р]?енус%s?+") or str:find("INFERNUS%s?+") then
  ...
  elseif not str:find("[Лл]ичным") and not str:find("[Gg][Pp][Ss]") and (str:find("Устроюсь") or str:find("[Тт]ранспортн[(ую)|(ои)]") or str:find("%s[Тт][Кк]%s?") or str:find("дальнобой") or str:find("%sтк%s") and not str:find("сутки")) then
  ...
  elseif str:find("[Pp]rice") and str:find("%s?11%s?%p%s?4") or str:find("8%s?%p?%s?197") or (str:find("[Нн]омерн(.-) [Зз]наки") and not str:find("[Лл][Сс]"))  or str:find("номера только в ЛВ") then
    --debug("+", 5)
    if str:find("[Pp]rice") then
      biz_loc = "Price 11-3"
    elseif str:find("[Gg][Pp][Ss]") then
  ...
  elseif not str:find('[Зз]наки') and (str:find("с[у]?[им][им]ку") or str:find("[Сс][иИ][Мм][(%p)|(карту)]") or str:find("[Сс][Мм][Ии] [Кк]арту") or str:find("[Ss][Iil]m") or str:find("%s[Сс]им%s") or str:find("SIM") or str:find("смку") or str:find("одам номер") or str:find("4 значный номер")) then -- ДРУГОЕ
    --формата "XY-XXXX".
  ...
  elseif str:find("[АаAa][Зз][СсCc]") or str:find("[Aa][Zz][Ss]") or str:find("заправочную станцию") then
    ...
    elseif str:find("Южная") or str:find("Южном") or str:find('Южную') then
      ...
      elseif str:find("[пП]род") then
        return "Продам «Южную АЗС №4» возле д. Angel Pine. "..get_price(str, trade_type(str))
      end
  ...
  elseif str:find("[Кк]леш") and str:find("[Кк]ра") then
    return action[trade_type(str)].."аксессуар «Клешни краба». "..get_price(str, trade_type(str))
  ...
  elseif str:find("[Дд]окт") and str:find("[Сс]тр[эе]") or str:find("[Пп][Лл][Аа][Щщ]") then
  ...
  elseif str:find("[Бб]оч") and str:find("[Тт]ок[сч]") then
    return action[trade_type(str)].."аксессуар «Бочка с токсическими отходами». "..get_price(str, trade_type(str))
  elseif str:find("[Мм]ар[жд]") and str:find("[Сс]им[пс]") then
    return action[trade_type(str)].."аксессуар «Мардж Симпсон». "..get_price(str, trade_type(str))
  elseif str:find("[Пп]ос") and str:find("[Дд]р[оу]") then
    return action[trade_type(str)].."аксессуар «Посох Друида». "..get_price(str, trade_type(str))
  ...
  elseif str:find("[бБ]ел[уао][яю] [Гг]итар[ау]") then
    return action[trade_type(str)].."аксессуар «Белая гитара». "..get_price(str, trade_type(str))
  elseif str:find("[чЧ][еоё]рн[уа][яю] [Гг]итар[ау]") then
    return action[trade_type(str)].."аксессуар «Чёрная гитара». "..get_price(str, trade_type(str))
  elseif str:find("[Гг]итар[ау]") and str:find("[Кк]рас") then
    return action[trade_type(str)].."аксессуар «Красная гитара». "..get_price(str, trade_type(str))
  ...
  elseif str:find("[Рр]юк[зс]ак") and str:find("[Дд]жинс") then
    return action[trade_type(str)].."аксессуар «Джинсовый рюкзак». "..get_price(str, trade_type(str))
  elseif (str:find("[Жж]елез") or str:find("[Мм]ета")) and str:find("[Кк]ры") then
    return action[trade_type(str)].."аксессуар «Железные крылья». "..get_price(str, trade_type(str))
  ...
  elseif str:find("[Рр]юкзак") and str:find("[Кк]расно%p?%s?[Бб]ел[ыи]й") then
  ...
  elseif str:find("[Оо]гнен") or str:find("[Шш]ар") or (str:find("[Rr]og") and str:find("[Ss]har")) then
  ...
  elseif str:find("[Зз]а[яй]чьи [Уу]ш[к]?и") or str:find("[Уу]ши [Зз]айца") or str:find("[Уу]шки [Зз]айки") then
  ...
  elseif (not str:find("резин") and not str:find("[бБ]юро")) and str:find("[Уу]див") or str:find("[Вв]згл") then
  ...
  elseif (str:find("[Аа]рмейский") or str:find("[Вв]о[єе]н[н]?[иы]")) and str:find("[Рр]ю[кд]зак") then
  ...
  elseif str:find("[сС]ер") and str:find("[Жж]елез") then
    return action[trade_type(str)].."аксессуар «Сердце железного человека». "..get_price(str, trade_type(str))
  ...
  elseif str:find("[ЭэЕе]лектрош") then
    return action[trade_type(str)].."аксессуар «Электрошокер». "..get_price(str, trade_type(str))
  ...
  elseif (str:find("[Пп]ерч") or str:find("[Рр]ук")) and str:find("[Тт]ан[оуа]с") then
    return action[trade_type(str)].."аксессуар «Перчатка Таноса». "..get_price(str, trade_type(str))
  elseif str:find("[Кк]атан") and (str:find("[Зз]олот") or str:find("[Гг]олд")) then
    return action[trade_type(str)].."аксессуар «Золотая катана». "..get_price(str, trade_type(str))
  elseif str:find("[Вв]инт") and str:find("[Сс]пи") or str:find("[Сс]найп") then
    return action[trade_type(str)].."аксессуар «Винтовка на спине». "..get_price(str, trade_type(str))
  elseif str:find("[Кк]атан") then
    return action[trade_type(str)].."аксессуар «Катана». "..get_price(str, trade_type(str))
  elseif str:find("[Нн][Лл][Оо]") then
    return action[trade_type(str)].."аксессуар «НЛО». "..get_price(str, trade_type(str))
  ...
  elseif not str:find("[Cc]lub") and str:find("[Сс]иг") or str:find("[Сс]и[гк]ар") or str:find("[Сс]иа[г]?ру") or str:find("[Сс]егару") then
  ...
  elseif str:find("[Чч]ас[иы]") and str:find("[Зз]олот") then
    return action[trade_type(str)].."аксессуар «Золотые часы». "..get_price(str, trade_type(str))
  ...
  elseif str:find("[Ff]am[ia]ly") or str:find("[Сс]ем[ь]?[яью]") or str:find("[^%p][Cc]lan") and not str:find("[Вв]ступлю") or str:find("поисках дальн") then
    ...
    elseif str:find("[Ии]щ[ую] семью$") or str:find("[Вв]ступлю в .+ семью") or str:find("[Вв]ступлю в семью") or str:find("вступить в семью") or str:find("Ищу семью.") or str:find("поисках дальн") or str:find("близких родст") or str:find("ищу родст[с]?вен[н]?иков") then
  ...
  elseif str:find("[Вв]рач") then
    if not str:find("LV %p") and str:find("[LlЛл][VvВв]") or str:find("[LlЛл][AaАа][SsСс]") then
  ...
  elseif str:find("портзал") or str:find("СПОРТЗАЛ") or str:find("боев") or str:find("зале") or str:find("отпор") or str:find("искусс") or str:find("бокс") or str:find("абонементы") or str:find("тзал") or str:find("стиль боя") or (str:find("спорт") and not str:find("прокат")) or str:find("Джеки Чан") or str:find("[Пп]одкач") or str:find("качаться") or str:find("%s[Зз]ал%p?%s") or str:find("[Кк]ачалк") then
  ...
  elseif str:find("[Кк]упл[юб]%s%d+,%d+,%d+") then
    return clothes(str, trade_type(str))
  elseif not str:find("[Мм][а]?газ") and not str:find("скидки") and not str:find("[Бб]из") and not str:find("[Нн]авигац") and str:find("[Оо]д[еж]?[еж][д]?") or str:find("[СсCc][КкИи][Рр]?[ИиКк][Нн]") or str:find("%sски %d+") or str:find("skin") or str:find("костю[ми][ы]?%s?[^(тел)]") or str:find("по[ш]?[иіi][вф]") or str:find("ПОШИВ") or str:find("бир[ко]") or str:find("шмот") or str:find("[Вв]уз[з]?и") or str:find("[рР]айдер") or str:find("[Бб]иг [Сс]мо") or str:find("[Bb]ig [Ss]mo") or str:find("[Шш]ахт[её]ра") or str:find("[Дд]жиз[з]?и") or str:find("[Мм]ясн") then
  ...
  elseif str:find("ищ[ую][^т]") or str:find("И[щш][ую][^т]") or str:find("[Вв]ступлю в [Сс]емью") or str:find("[Вв]ступлю в фаму") or str:find("позвони мне") or str:find("еловек по имени") or str:find("[Вв] поисках") or str:find("[Нн]айду") then
    -- debug("FIND:"..str, 1)
    if str:find("_") then str = str:gsub("_", " ") end
    if str:find("[ч]?елов[е]?[к]?[а]?") or str:find("игрока") or (str:find("позвони мне") and not str:find("альних")) or str:find("[Ии][шщ][ую]%s%w+%s%w+$") then
      debug(str, 1)
      ...
      elseif str:find("[(имени)|(ником)|(именем)|(імені)|(игрока)]%s%p?(%w+)%s(%w+)%p?%s?%p?") then
        ...
        elseif str:find("[иі]мен[иі] (%w+) (%w+)%s[Сс]вяж") then
          ...
          debug("P2, ID - 3", 1)
        elseif str:find("[иі]мен[н]?[иі] (%w+) (%w+)%p?%s%W+") then
          first, last = str:match("[иі]мен[н]?[иі] (%w+) (%w+)%p?%s%W+")
          debug("P2, ID - 4", 1)
        elseif str:find("именем%s%w+%s%w+.%sПозвони") then
          debug("P2, ID - 5", 1)
          first, last = str:match("именем (%w+)%s(%w+).%sПозвони")
        elseif str:find("щу человека %w+ %w+. Поз") then
          debug("P2, ID - 6-1", 1)
          first, last = str:match("щу человека (%w+) (%w+). Поз")
    ...
    elseif str:find("родс[т]?[с]?[т]?в[уи]?[ен][ен]") or str:find("близких родст") or str:find("[Сс]емью") or str:find("фаму") then
      return "Ищу дальних родственников. Позвоните мне!"
    else
      if str:find("щу (.+) (.+)") then
        --щу Dmitriy Forost.
  ...
  elseif str:find("(.-)о[дж][ам][ма]") or str:find("[Ss]ell") or str:find("[Пп]роадм") or str:find("(.-)родаю") or str:find("одат") or str:find("продам") or str:find("[Аа]ук") then
    ...
    elseif str:find("[ЭэЕе]кск") or str:find("[тТ]рансп") then
      return "Продам бизнес «Прокат эксклюзивных авто»"..get_location(str)..". "..get_price(str, trade_type(str))
    ...
    elseif str:find("[Зз]ак[у]?сочн") or str:find("[Зз]акуск[уа]") or str:find("[Пп]ончик") or str:find("Burger%s?Shot") or str:find("[Бб]у[рг][гре][ег][о]?%s?[шШ]от") or str:find("[Бб]ур[гш]ер [Шш]от") then
      ...
      elseif str:find("аукционе кондитерская") then
        return "На аукционе кондитерская «Пончик» за больницей ш. LS. "..get_price(str, trade_type(str))
      ...
      elseif str:find("[Зз]акусочн[ау][яю] Burger Shot") and str:find("№1") then
        if str:find("ау[кц]") then
          return "На аукционе бизнес «Burger Shot №1» "..get_location(str)..". "..get_price(str, trade_type(str))
        end
        return "Продам закусочную «Burger Shot №1» "..get_location(str)..". "..get_price(str, trade_type(str))

      elseif str:find("[Зз]акусочн[ау][яю] Burger Shot") and str:find("№2") then
        if str:find("ау[кц]") then
          return "На аукционе бизнес «Burger Shot №2» "..get_location(str)..". "..get_price(str, trade_type(str))
        end
        return "Продам закусочную «Burger Shot №2» "..get_location(str)..". "..get_price(str, trade_type(str))

      elseif (str:find("[Зз]акусочн[ау][яю] Burger Shot") and str:find("№3")) or str:find("[Bb]urger [Ss]hot №3") then
        if str:find("ау[кц]") then
          return "На аукционе бизнес «Burger Shot №3» "..get_location(str)..". "..get_price(str, trade_type(str))
        end
        return "Продам закусочную «Burger Shot №3» "..get_location(str)..". "..get_price(str, trade_type(str))
      ...
      return "Продам закусочную"..get_location(str)..". "..get_price(str, trade_type(str))
    ...
    elseif str:find("[Мм][а]?гази[нг] [Оо]дежды") or str:find("магаз одежды") or str:find("биз одежда") or str:find("%s[DД][SСC]") or str:find("[Uu]rban") then
    ...
    elseif str:find("[Рр]есторан") then
      ...
      elseif str:find("[Аа]ле[ей]") then
        return "Продам «Ресторан на алее звёзд» в ш. Los Santos. "..get_price(str, trade_type(str))
      end
    ...
    elseif str:find("[Нн]омерн(.+) знаки") then
      if str:find("[ЛлlL][СсSs]") then
        return "Продам бизнес «Номерные знаки в LS». "..get_price(str, trade_type(str))
      elseif str:find("[(SF)|(СФ)|[сф]]") then
        return "Продам бизнес «Номерные знаки в SF». "..get_price(str, trade_type(str))
      elseif str:find("[LlЛл][VvВв]") then
        return "Продам бизнес «Номерные знаки в LV». "..get_price(str, trade_type(str))
      end
  ...
  elseif str:find("[Рр][Аа]бо[ат]") and str:find("[Мм]ехан") then
    return "По федерации работает опытный автомеханик. Звоните."
...
function trade_type(str)
  if str:find("(.-)[УуyY][цЦ]?[плИи][а]?[лп][дб]?ю") or str:find("[Кк]упл.") or str:find("[Кк]упаю") or str:find("[kK][uy]pl[y]?[uy]") or str:find("[Кк]упить") or str:find("[Пп]риобре") or str:find("^[Кк]пю") or str:find("(.-)[Уу][Пп][лЛДд][юЮБб]") or str:find("kyply") or str:find("[Кк]у[а]?лю") or str:find("[Bb]uy") or str:find("[Кк]алю") or str:find("[Кк]упю") then
    print(str)
    return "buy"
  elseif str:find("(.-)[оoл][дd][aав][мmy]") or str:find("[Пп]ро[ак]дм") or str:find("[Пп]Р[Р]?[ОЛ]Д[Ж]?[Аа][мМ]") or str:find("^[Пп]ро[дД]") or str:find("[Pp]rodam") or str:find("(.-)родаю") or str:find("(.*)ро(.*)[ап]м") or str:find("одат") or str:find("рдам") or str:find("родм") or str:find ("[Пп]родма") or str:find ("[Пп][рР][Оо]да") or str:find("(.-)ell") or str:find("^[Сс][Ее][Лл][Лл]") or str:find("[Gg]hjlfv") then
    return "sell"
...
function get_taxi_name(str)
  ...
  return string.format(" «%s»", taxi_n or "")
end
...
function get_simcard_fmt(str)
  ...
  elseif str:find("типа%s[^(Номер)].-$") then
...
function clothes(str, act)
  ...
  elseif str:find('[Кк]упл[юб]%s%d+%s%d+%s%d+$') then
    clothes_ids = get_clothes_id(str, "[Кк]упл[юб]%s(%d+)%s(%d+)%s(%d+)$", 3, 320)
    h_string = action[act].."одежду с бирками "..clothes_ids..". "..get_price(str, act) 
  ...
  elseif str:find("skin:%d+,%d+,%d+$") then
    ...
    h_string = action[act].."одежду с бирками "..clothes_ids..". "..get_price(str, act)
  elseif str:find("%W+%s%d+%s%d+%s%d+%s%W+") then
    --продам скины 111 112 125 Цена 9кк
    clothes_ids = get_clothes_id(str, "%W+%s(%d+)%s(%d+)%s(%d+)%s%W+", 3, 327)
    h_string = action[act].."одежду с бирками "..clothes_ids..". "..get_price(str, act)
  elseif str:find("%s%d+%p%s№%d+%p%s№%d+") then
    --продам скины 111 112 125 Цена 9кк
    clothes_ids = get_clothes_id(str, "%s(%d+)%p%s№(%d+)%p%s№(%d+)", 3, 327)
    h_string = action[act].."одежду с бирками "..clothes_ids..". "..get_price(str, act)
  elseif str:find("%W+%s%W+%s№%d+%s%p%s№%d+%sи%s№%d+$") then
    --костюм пошива №125 , №83 и №164
    clothes_ids = get_clothes_id(str, "%W+%s%W+%s№(%d+)%s%p%s№(%d+)%sи%s№(%d+)$", 3, 327)
    h_string = action[act].."одежду с бирками "..clothes_ids..". "..get_price(str, act)
  ...
  elseif str:find("№%d+%sи%s№%d+$?") then
    ...
    h_string = action[act].."одежду с бирками "..clothes_ids..". "..get_price(str, act)
  elseif str:find("№%s%d+%sи%s№%s%d+$?") then
    clothes_ids = get_clothes_id(str, "№%s(%d+)%sи%s№%s(%d+)$?", 2, 217)
    h_string = action[act].."одежду с бирками "..clothes_ids..". "..get_price(str, act)
  elseif str:find("№%d+%p№%d+$?") then
...
function vechicles(str, trade, v_type, model)
  ...
  if str:find("рынок") or str:find("р[ыи]нке") or str:find("укци") or str:find("[Вв]ы[с]?%p?тавл[ея]н") then
    return "На авторынке выставлен "..v_type.." марки «"..model.."»"..car_tuning(str).." "..get_price(str, "carmarket")
  ...
  print("{AC41BF}"..a)
  return action[trade]..v_type.." марки «"..model.."»"..car_tuning(str)..". "..(get_price(str, trade) or 0)
end
...
function get_price(str, act)
  ...
  elseif str:find("[цйЦ][еЕ][нН][Аае]") or str:find ("за %d") or str:find("%d+кк") or str:find("%d КК$") or str:find("[Кк][Кк][^Сс]") or str:find("Цена") or str:find("ю[дт]жет") or str:find("%d[кk]$") or str:find("[cC]ena") or str:find("[^ea][kr][kr]") and not str:find("договорная") or act == "surcharge" then
    ...
    elseif str:find("%s%dлям") then price = ''..str:sub(str:find("%s%dл")+1, str:find("лям")-1)..".000.000$" k = 11
    ...
    elseif str:find("%s%dлям") then price = ''..str:sub(str:find("%s%dл")+1, str:find("лям")-1)..".000.000$" return act_text[act]..price
    ...
    elseif str:find("%d+КК$") then price = ''..(str:sub(str:find("%s%d+К")+1, str:find("%dКК$")) or "error")..".000.000$"  k = 4 return act_text[act]..price
      -- фт 4.8
